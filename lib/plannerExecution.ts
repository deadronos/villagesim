...
const DEFAULT_HOSTED_PLANNER_DISPATCH_LEASE_MS = 30_000;
...
export function readHostedPlannerDispatchLeaseMs(): number {
  return readPositiveInt(process.env.VILLAGESIM_PLANNER_DISPATCH_LEASE_MS, DEFAULT_HOSTED_PLANNER_DISPATCH_LEASE_MS);
}

function createDefaultPlannerState(): TownPlannerState {
  return {
    queue: [],
    lastTickBudget: null,
    metrics: {
...
function refreshPlannerQueueCounts(plannerState: TownPlannerState) {
  plannerState.metrics.queuedCount = plannerState.queue.filter((item) => item.status === "queued").length;
  plannerState.metrics.dispatchingCount = plannerState.queue.filter((item) => item.status === "dispatching").length;
  return {
    queued: plannerState.metrics.queuedCount,
    dispatching: plannerState.metrics.dispatchingCount,
    remaining: plannerState.metrics.queuedCount + plannerState.metrics.dispatchingCount,
  };
}

function cloneQueueEntry<T>(value: T): T {
  return JSON.parse(JSON.stringify(value)) as T;
}

export function ensurePlannerState(town: TownState): TownPlannerState {
  const defaultState = createDefaultPlannerState();
  if (!town.metadata.planner) {
    town.metadata.planner = defaultState;
    return town.metadata.planner;
  }

  const plannerState = town.metadata.planner;
  plannerState.queue = plannerState.queue ?? [];
  plannerState.lastTickBudget = plannerState.lastTickBudget ?? null;
  plannerState.metrics = {
    ...defaultState.metrics,
    ...(plannerState.metrics ?? {}),
    completedBySource: {
      ...defaultState.metrics.completedBySource,
      ...(plannerState.metrics?.completedBySource ?? {}),
    },
    lastFallbackReason: plannerState.metrics?.lastFallbackReason ?? null,
  };

  refreshPlannerQueueCounts(plannerState);
  return plannerState;
}
...
function isDispatchClaimStale(item: PlannerQueueEntry, now: number, leaseMs: number): boolean {
  if (item.status !== "dispatching") {
    return false;
  }
  return now - (item.dispatchStartedAt ?? 0) >= leaseMs;
}

export function claimHostedPlannerQueueEntry(
  town: TownState,
  options: {
...
): PlannerQueueEntry | null {
  const plannerState = ensurePlannerState(town);
  const leaseMs = readHostedPlannerDispatchLeaseMs();
  const item = plannerState.queue.find((entry) => entry.status === "queued" || isDispatchClaimStale(entry, options.now, leaseMs));
  ...
  item.dispatchToken = options.dispatchToken;
  item.dispatchStartedAt = options.now;
  item.dispatchAttemptCount = (item.dispatchAttemptCount ?? 0) + 1;
  item.lastDispatchSource = options.source;
  refreshPlannerQueueCounts(plannerState);
  return cloneQueueEntry(item);
}

function clearDispatchLease(item: PlannerQueueEntry) {
  item.dispatchToken = undefined;
  item.dispatchStartedAt = undefined;
}

type SettledQueueEntryResult =
  | {
      appliedPlan: boolean;
      dispatching: number;
      outcome: "completed";
...
export function completeHostedPlannerQueueEntry(
  town: TownState,
  options: {
    completedAt: number;
    queueId: string;
...
): SettledQueueEntryResult {
  const plannerState = ensurePlannerState(town);
  const item = plannerState.queue.find((entry) => entry.id === options.queueId);

  if (!item || item.status !== "dispatching" || item.dispatchToken !== options.token) {
    const counts = refreshPlannerQueueCounts(plannerState);
    return {
      outcome: "stale",
      ...counts,
    };
  ...
  item.completedAt = options.completedAt;
  item.source = options.result.source === "queued-placeholder" ? "mock" : options.result.source;
  item.latencyMs = options.result.latencyMs;
  item.failureReason = options.result.failureReason ?? null;
  item.fallbackReason = options.result.fallbackReason ?? null;
  clearDispatchLease(item);
  ...
  plannerState.metrics.lastCompletionAt = options.completedAt;
  plannerState.metrics.lastCompletionLatencyMs = item.latencyMs;
  if (item.fallbackReason) {
    plannerState.metrics.totalFallbacks += 1;
    plannerState.metrics.lastFallbackReason = item.fallbackReason;
  }
  ...
  if (applyPlan) {
    assignPlanToTown(town, item.npcId, {
      ...options.result.plan,
      planner: {
        source: options.result.plan.planner?.source ?? options.result.source,
        queueId: item.id,
        requestedAt: options.result.plan.planner?.requestedAt,
        completedAt: options.result.plan.planner?.completedAt,
        latencyMs: options.result.plan.planner?.latencyMs,
        fallbackReason: options.result.plan.planner?.fallbackReason,
        failureReason: options.result.plan.planner?.failureReason,
      },
    });
...
export function failHostedPlannerQueueEntry(
  town: TownState,
  options: {
    completedAt: number;
    error: unknown;
  ...
  item.status = "failed";
  item.completedAt = options.completedAt;
  item.failureReason = options.error instanceof Error ? options.error.message : "Unknown planner queue failure";
  clearDispatchLease(item);
  plannerState.metrics.totalFailed += 1;
  plannerState.metrics.lastFailureAt = options.completedAt;
...
export function recordHostedPlannerDispatchMetrics(
  town: TownState,
  options: {
    completedAt: number;
    processed: number;
    result: "success" | "partial" | "noop" | "failed";
    source: PlannerDispatchSource;
    startedAt: number;
  },
) {
  const plannerState = ensurePlannerState(town);
  const counts = refreshPlannerQueueCounts(plannerState);
  plannerState.metrics.lastDispatchAt = options.completedAt;
  plannerState.metrics.lastDispatchStartedAt = options.startedAt;
  plannerState.metrics.lastDispatchCompletedAt = options.completedAt;
  plannerState.metrics.lastDispatchDurationMs = Math.max(0, options.completedAt - options.startedAt);
  plannerState.metrics.lastDispatchSource = options.source;
  plannerState.metrics.lastDispatchResult = options.result;
  plannerState.metrics.lastDispatchProcessed = options.processed;
  plannerState.metrics.lastDispatchRemaining = counts.remaining;
  return counts;
}
